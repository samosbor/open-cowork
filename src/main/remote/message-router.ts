...
export class MessageRouter {
   ...
   * Supports a [cwd:<path>] prefix to specify the working directory
   * Also supports a !cd <path> command to change the working directory
   */
  private extractPromptAndCwd(message: RemoteMessage): { prompt: string; cwd?: string } {
    let cwd: string | undefined;
    
    if (message.content.type === 'text' && message.content.text) {
      // Remove internal mention placeholders used by some channels
