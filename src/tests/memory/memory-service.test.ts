...
class MockMemoryLLMClient implements MemoryLLMClientLike {
  async complete(request: MemoryCompletionRequest): Promise<{ text: string }> {
    ...
    if (
      ...
      if (transcript.includes('gateway token rotation')) {
        return {
          text: JSON.stringify({
            ...
            chunks: [
              {
                summary: 'Implement the main gateway token rotation changes',
                details: 'Recorded implementation details of gateway token rotation and synchronized remote gateway behavior.',
      ...
      return {
        text: JSON.stringify({
          session_summary: 'Record stable user preferences',
          session_keywords: ['preference'],
          chunks: [
            {
              summary: 'User stated a preference for Chinese responses',
              details: 'The conversation explicitly requested Chinese as the default language.',
...
describe('MemoryService', () => {
  ...
  it('writes core and unified experience memory into JSON files', async () => {
    ...
    const messages = makeMessages('session-a', [
      { role: 'user', text: 'Please reply in Chinese, my name is Jack.', timestamp: 1 },
      { role: 'assistant', text: 'Okay, I will continue in Chinese.', timestamp: 2 },
      {
        role: 'user',
  ...
  it('builds progressive prompt context and supports search/read/debug inspection', async () => {
    await service.enqueueIngestion({
      ...
      messages: makeMessages('session-a', [
        { role: 'user', text: 'Please reply in Chinese.', timestamp: 1 },
        { role: 'assistant', text: 'Okay.', timestamp: 2 },
        {
          role: 'user',
          text: 'Implement gateway token rotation in workspace A and synchronize remote gateway.',
          timestamp: 3,
        },
        {
          role: 'assistant',
  ...
  it('searches all source workspaces when scope is all even with a current cwd', async () => {
    ...
    await service.enqueueIngestion({
      ...
      messages: makeMessages('session-b', [
        {
          role: 'user',
          text: 'Implement gateway token rotation in workspace B and synchronize remote gateway.',
          timestamp: 3,
        },
        {
          role: 'assistant',
  ...
  it('rebuilds all memory from persisted sessions and messages', async () => {
    ...
    insertMessage(rawDb, {
      ...
      sessionId: 'session-a',
      role: 'user',
      text: 'Please reply in Chinese, my name is Jack.',
      timestamp: 1,
    });
    insertMessage(rawDb, {
      id: 'm2',
  ...
  it('does not resurrect deleted experience memory when deletion happens during queued ingestion', async () => {
    ...
    const messages = makeMessages(sessionId, [
      ...
      {
        role: 'user',
        text: 'Continue handling gateway token rotation and record remote gateway constraints.',
        timestamp: 3,
      },
      { role: 'assistant', text: 'Gateway token rotation is complete.', timestamp: 4 },
    ]);
