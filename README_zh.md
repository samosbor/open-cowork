...
- **远程控制**：可以接入 **Telegram** 等协作平台和远程服务，实现工作流自动化和跨平台操作。
...
Open Cowork 是 Claude Cowork 的开源实现，在此基础上增加了多模型支持（不只是 Claude）、GUI 自动化操作、Telegram/Slack 远程控制，以及虚拟机级别的沙盒隔离。详见[功能对比表](#核心特性)。
...
**如何设置 Telegram 或 Slack 远程控制？**
Open Cowork 支持通过 Telegram 和 Slack 进行远程控制，你可以在协作平台上发送指令并接收执行结果。请在应用设置中查看远程控制配置选项。
