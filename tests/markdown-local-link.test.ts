...
describe('normalizeLocalFileMarkdownLinks', () => {
  it('normalizes absolute macOS path markdown links with spaces and newlines', () => {
    const input = [
      ...
      '/Users/haoqing/Library/Application Support/open-cowork/default_working_dir/beijing-weather-next-month.docx',
      ')',
    ].join('\n');

    const output = normalizeLocalFileMarkdownLinks(input);

    expect(output).toContain('[beijing\\-weather\\-next\\-month.docx](file:///Users/haoqing/Library/Application%20Support/open-cowork/default_working_dir/beijing-weather-next-month.docx)');
  });
...
describe('extractLocalFilePathFromHref', () => {
  it('extracts decoded local path from file URL', () => {
    const href = 'file:///Users/haoqing/Library/Application%20Support/open-cowork/test.docx';
    expect(extractLocalFilePathFromHref(href)).toBe('/Users/haoqing/Library/Application Support/open-cowork/test.docx');
  });

  it('extracts UNC paths from file URLs without dropping the host', () => {
    const href = 'file://server/share/test.docx';
    const result = extractLocalFilePathFromHref(href);
    if (process.platform === 'win32') {
      expect(result).toBe('\\\\server\\share\\test.docx');
    } else {
      expect(result).toBe('//server/share/test.docx');
    }
  });
...
describe('resolveLocalFilePathFromHref', () => {
  it('resolves relative artifact links using cwd', () => {
    const href = 'reports/beijing-weather-next-month.docx';
    expect(resolveLocalFilePathFromHref(href, '/Users/haoqing/Library/Application Support/open-cowork/default_working_dir'))
      .toBe('/Users/haoqing/Library/Application Support/open-cowork/default_working_dir/reports/beijing-weather-next-month.docx');
  });
